...
class ClientHandler(threading.Thread):
    ...
    def run(self):
        ...
        logging.info(f"client connected from {self.addr}")
        try:
            while True:
                # read header
                ...
                logging.info(f"header received Version: {version}, Code: {code}, Payload Size: {payload_size}")
            ...
            logging.error(f"failed to receive header from {self.addr}")
    ...
    def _route_request(self, code: int, client_id: bytes, payload:bytes):
        ...
        match req_code:
            ...
            case RequestCode.CRCBadResend:
                logging.info(f"client {client_id.hex()} reported mismatch CRC, waiting for resend")
    ...
    def _send_response(self, code: ResponseCode, payload: bytes =b''):
        ...
        try:
            ...
            logging.info(f"sent response: Code: {code.value}, Payload Size: {len(payload)}")
        except Exception as e:
            logging.error(f"failed to send response to {self.addr}: {e}")
    ...
    def handle_registration(self, client_id: bytes, payload: bytes):
        ...
        logging.info(f"client registered: {client_id} starting...")
        try:
            name_bytes = payload.split(b'\0',1)[0]
            name = name_bytes.decode('ascii')

            if not name:
                logging.warning("empty name received")
            ...
            if self.db.client_exists(name):
                ...
                logging.info(f"new client {name} registered with UUID: {new_client_id}")
                self._send_response(ResponseCode.RegisterSuccess, new_client_id)
        except UnicodeDecodeError:
            logging.error("failed to decode client name")
            self._send_response(ResponseCode.RegisterFail)
        except Exception as e:
    ...
    def handle_public_key(self, client_id: bytes, payload: bytes):
        ...
        logging.info(f"processing public key for client {client_id}...")
        try:
            ...
            if not self.db.update_client_key(client_id, public_key_bytes, aes_key):
                logging.error(f"failed to update '{name}' keys")
                self._send_response(ResponseCode.GeneralError)
                return
            # encrypted aes key
            ...
            logging.info(f"successfully received public key for client {client_id}")

        except UnicodeDecodeError:
            logging.error("failed to decode client name")
            self._send_response(ResponseCode.GeneralError)
        except Exception as e:
            logging.error(f"error handling public key request for client {client_id}: {e}")
            self._send_response(ResponseCode.GeneralError)

    def handle_reconnect(self, client_id: bytes, payload: bytes):
        """handle reconnect request (827)"""
        logging.info(f"client {client_id} reconnecting...")
        try:
            ...
            if not public_key_bytes:
                logging.warning(f"client {client_id} not found or missing public key")
            ...
            logging.info(f"successfully reconnected to client {client_id}")
        except UnicodeDecodeError:
            logging.error("failed to decode client name in reconnect")
            self._send_response(ResponseCode.GeneralError)
        except Exception as e:
            logging.error(f"failed to reconnect to client {client_id}: {e}")
            self._send_response(ResponseCode.GeneralError)
    ...
    def handle_send_file(self, client_id: bytes, payload: bytes):
        ...
        if not handled_payload:
            self._send_response(ResponseCode.GeneralError)
            return
        content_size, packet_num, total_packets, file_name, encrypted_data = handled_payload

        try:
            file_path = FileManager.save_chunk(client_id, file_name,packet_num, encrypted_data)
            logging.info(f"packet {packet_num}/{total_packets} for file {file_name} saved")

            if packet_num == total_packets:
                logging.info(f"all packets for file {file_name} saved")
                ...
                logging.info(f"file {file_name} successfully transferred")
        except Exception as e:
            logging.error(f"failed to transfer file {file_name}: {e}")
            self._send_response(ResponseCode.GeneralError)

    def handle_crc_ok(self, client_id: bytes, payload: bytes):
        """handle CRC ok massage (900)"""
        if len(payload) < Database.NAME_LEN:
            logging.warning(f"invalid payload length for CRC OK: {len(payload)}")
            self._send_response(ResponseCode.GeneralError)
            return

        file_name = payload[:Database.NAME_LEN].split(b'\0',1)[0].decode('ascii')
        file_name = os.path.basename(file_name)
        logging.info(f"client {client_id.hex()} confirmed CRC for file {file_name}")
    ...
    def handle_crc_abort(self, client_id: bytes, payload: bytes):
        ...
        if len(payload) < Database.NAME_LEN:
            logging.warning(f"invalid payload length for CRC ABORT: {len(payload)}")
            self._send_response(ResponseCode.GeneralError)
            return

        file_name = payload[:Database.NAME_LEN].split(b'\0',1)[0].decode('ascii')
        file_name = os.path.basename(file_name)
        logging.info(f"client {client_id.hex()} aborted transfer file {file_name}")
